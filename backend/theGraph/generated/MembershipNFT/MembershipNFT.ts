...
export class MembershipNFT extends ethereum.SmartContract {
  ...
  issueToken(to: Address): BigInt {
    let result = super.call("issueToken", "issueToken(address):(uint256)", [
  ...
  try_issueToken(to: Address): ethereum.CallResult<BigInt> {
    let result = super.tryCall("issueToken", "issueToken(address):(uint256)", [
      ethereum.Value.fromAddress(to)
    ]);
    if (result.reverted) {
      return new ethereum.CallResult();
